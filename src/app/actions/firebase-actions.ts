...
export async function getReservations(day: Date, schedule: "lunch" | "dinner") {
	...
	await db.collection("reservations").where("start", ">=", startTime).where("end", "<=", endTime).get()
		.then((querySnapshot) => {
...
export async function getSettings() {
	return db.collection("settings").doc("1").get().then((doc) => {
...
export async function updateOneSettings(key: string, value: any) {
	db.collection("settings").doc("1").update({
		[key]: value
	});
}