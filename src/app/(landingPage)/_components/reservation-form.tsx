...
export default function ReservationForm() {
  ...
  const [formValues, setFormValues] = useState<Reservation>({
    guestNumber: 1,
    start: 0,
    end: 0,
    name: "",
  ...
  const [schedule, setSchedule] = useState<"lunch" | "dinner">("lunch");
  const [slot, setSlots] = useState<string>("");
  ...
  const handleScheduleChange = (schedule: "lunch" | "dinner") => {
  ...
  useEffect(() => {
    if (step === 4) {
      getAvailableSlots(date, schedule, formValues.guestNumber).then((availableSlots) => {
    ...
    <div className="max-w-md mx-auto w-full p-4">
      ...
      {step === 1 && (
        <div className="space-y-4 flex flex-col justify-start text-left">
          ...
          <Button className="text-lg h-16 pointer" onClick={() => handleScheduleChange("lunch")}>
            Lunch
          </Button>
          <Button className="text-lg h-16 pointer" onClick={() => handleScheduleChange("dinner")}>
            Middag
          </Button>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="w-full flex flex-col">
          <h3 className="text-xl font-semibold text-center">{scheduleInSwedish[schedule]}</h3>
          <div className="h-[1px] bg-primary my-4" />
      ...
      {step === 3 && (
        <div className="flex flex-col justify-center items-center">
          <h3 className="text-xl font-semibold text-center mb-2">{scheduleInSwedish[schedule]}</h3>
          ...
          <Calendar onClickDay={(date) => handleDateChange(date)} minDate={new Date()} locale="sv" className="border p-2 rounded w-full" />
        </div>
      )}
        ...
        <div className="">
          ...
          <div className="flex justify-center gap-2 flex-col overflow-y-scroll">
            {availableSlots.map((time, i) => (
              <Button key={i} onClick={() => handleSlotChange(toSwedishTime(time))}>
                {toSwedishTime(time)}
              </Button>
            ))}
          </div>
      ...
      {step === 5 && (
        <div>
          <h3 className="text-xl font-semibold text-center mb-2">{schedule}</h3>
          <div className="flex gap-4 justify-center">
            <Guests guestNumber={formValues.guestNumber} />
            <div className="text-center flex items-center gap-2">
              <Calendar1 className="w-5 h-5 " strokeWidth={1} />
